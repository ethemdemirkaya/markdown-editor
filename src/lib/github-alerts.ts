...
// --- Marked extension (export için) ---

function detectAlertType(token: any): AlertType | null {
  if (token.type !== 'blockquote' || !Array.isArray(token.tokens) || token.tokens.length === 0)
    return null;
  const first = token.tokens[0];
  if (first.type !== 'paragraph' || !first.text) return null;
  const m = ALERT_REGEX.exec(first.text);
  if (!m) return null;
  return m[1].toLowerCase() as AlertType;
}

export const markedGithubAlerts = {
  walkTokens(token: any) {
    const alertType = detectAlertType(token);
    if (!alertType) return;
    token.alertType = alertType;
    const first = token.tokens[0];
    // İlk satırı (örn. "[!NOTE]\n") temizle
    first.text = first.text.replace(ALERT_REGEX, '');
    // İçindeki text token'larını yeniden oluştur (marked v18 nested tokens)
    if (Array.isArray(first.tokens) && first.tokens.length > 0) {
      const firstSub = first.tokens[0];
      if (firstSub && firstSub.type === 'text' && typeof firstSub.text === 'string') {
        firstSub.text = firstSub.text.replace(ALERT_REGEX, '');
        firstSub.raw = firstSub.text;
      }
    }
  },
  renderer: {
    blockquote(this: any, token: any): string {
      const alertType: AlertType | undefined = token.alertType;
      const inner = this.parser.parse(token.tokens);
      if (alertType) {
        return `<div class="md-alert md-alert-${alertType}"><div class="md-alert-title">${ICONS[alertType]}<span>${TITLES[alertType]}</span></div><div class="md-alert-body">${inner}</div></div>`;
      }
...
// --- ProseMirror plugin (WYSIWYG için) ---

const KEY = new PluginKey('github-alert-decoration');

function detectAlertFromNode(node: any): AlertType | null {
  if (node.type.name !== 'blockquote') return null;
  const first = node.firstChild;
  if (!first || first.type.name !== 'paragraph') return null;
  const m = ALERT_REGEX.exec(first.textContent);
  if (!m) return null;
  return m[1].toLowerCase() as AlertType;
}

export const githubAlertDecoration = new Plugin({
  key: KEY,
  props: {
    decorations(state) {
      const decos: Decoration[] = [];
      state.doc.descendants((node, pos) => {
        const alertType = detectAlertFromNode(node);
        if (!alertType) return;
        decos.push(
          Decoration.node(pos, pos + node.nodeSize, {
            class: `md-alert-bq md-alert-bq-${alertType}`,
            'data-alert-type': alertType,
            'data-alert-title': TITLES[alertType],
          }),
        );
        return false;
      });
